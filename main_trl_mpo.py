...
from buffer.gc_episodic_replaybuffer import GoalConditioned_EpisodicReplayBuffer
...
from trl.scripts.TRL_MPO_Learner import TRL_with_MPO_Learner
...
def find_gc_obs_wrapper(env):
        current = env
        while True:
            if isinstance(current, GC_Obs_Wrapper):
                return current
            if not hasattr(current, "env"):
                break
            current = current.env
        raise TypeError(f"GC_Obs_Wrapper not found in wrapper chain: {type(env)}")

def make_replaybuffer(args, obs_wrapper):
    # Create ReplayBuffer
    ...
    replaybuffer = GoalConditioned_EpisodicReplayBuffer(args.max_buffer_capacity, obs_wrapper, args.obs_dim, args.action_dim, device_buffer)
...
def train():
    ...
    # Create Replaybuffer
    gc_wrapper = find_gc_obs_wrapper(eval_env)
    replaybuffer = make_replaybuffer(args, gc_wrapper)
   
    # Create MPO Policy Optimizer
    trl_mpo = TRL_with_MPO(args, eval_env, gc_wrapper, actor, target_actor, critic, target_critic, actor_optimizer, critic_optimizer, device) 
    ...
    TRL_with_MPO_Learner(args, train_env, eval_env, device, replaybuffer, trl_mpo, writer)
