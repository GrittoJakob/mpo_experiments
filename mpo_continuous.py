...
"""
Changes from discrete to continuous action:

1. log prob statt probs
2. Änderungen in der Update critic retrace funktion: statt discrete policy Distribution over actions verwenden.
3. Geänderter Formel der dualen Funktion aufgrund log_prob.
4. Hilfverteilung b_prob_log neu coden!
4. Monte Carlo integral statt summe für val in critic retrace update
5. KL divergence from categorical zu continuous ändern
6. 
...
class MPO(object):
    """
        Maximum A Posteriori Policy Optimization (MPO) ; Continuous action-space ; Retrace
    ...
    def __init__(self, env, actor, critic, obs_shape, action_shape,
                 ...
                 episodes=1000, sample_episodes=1, episode_length=1000, num_action_samples = 64
        ...
        self.runs = runs
        self.device = device
        self.num_action_samples = number_action_samples
    ...
    # take log_prob of actions instead prob
    def _sample_trajectory(self):       
        ...
        for _ in range(self.sample_episodes):
            ...
            log_prob_b = np.zeros([self.episode_length, self.env.num_envs, self.action_shape])
            done_b = np.zeros([self.episode_length, self.env.num_envs])

            for steps in range(self.episode_length):
                action, log_prob = self.target_actor.action(torch.from_numpy(np.expand_dims(obs, axis=0)).to(self.device).float())
                ...
                log_prob_b[steps] = log_prob
            
            self.buffer.put(obs_b, action_b, reward_b, log_prob_b, done_b)

        return mean_reward / self.episode_length / self.sample_episodes

    def _update_critic_retrace(self, state_batch, action_batch, policies_batch, reward_batch, done_batch, sample_num):
        
        with torch.no_grad():
            action_distribution = self.actor.dist(state_batch)
            sample_next_actions = action_distribution((num_sample,)).transpose(0,1)         # (B, sample_num, dim_act)
            expanded_next_states = next_state_batch[:, None, :].expand(-1, sample_num, -1)  # (B, sample_num, dim_state)

            expected_Q = self.target_critic.forward(
            ...
            expected_Q_mean = expected_Q.mean(dim = 1)
        ...
        with torch.no_grad():
            dist, a_log_prob, entropy = self.actor.evaluate_action(state_batch.view(-1, self.obs_shape), action_batch.view(-1, 1))
            target_dist, _, _ = self.target_actor.evaluate_action(state_batch.view(-1, self.obs_shape), action_batch.view(-1, 1))
        

        qval = self.critic(state_batch.view(-1, self.obs_shape), action_batch.view(-1, self.action_shape))
        #val = (qval * dist).sum(1, keepdim=True)    # noch anzupassen
        #statt summe monte carlo integral

        old_policies = policies_batch.view(-1, action_size)
        dist = dist.view(-1, action_size)
        target_dist = target_dist.view(-1, action_size)
        ...
        with torch.no_grad():
            next_qval = self.critic(state_batch_last, action_batch).detach()
            next_policies = self.actor.get_action_log_prob(state_batch_last, action_batch).detach()
    ...
    """
    #change KL-divergence to continuous divergence
    ...
    def train(self):
        ...
        for episode in range(self.episodes):
            ...
            mean_rewaber ard = self._sample_trajectory()
            mean_q_loss = 0
            mean_policy = 0
            ...
            for _ in range(self.runs):
                ...
                B, dim_state = state_batch.shape
                dim_act = env.action_space.shape[0]

                with torch.no_grad():
                    """
                    actions = torch.arange(self.action_shape)[..., None].expand(self.action_shape, self.mb_size).to(self.device)
                    b = self.target_actor.dist(state)
                    b_log_prob = b.expand((self.action_shape, self.mb_size)).log_prob(actions).exp()
                    target_q = self.target_critic.(state_batch, actions_batch)
                    target_q = target_q.transpose(0, 1)
                    b_log_prob_np = b_log_prob.cpu().numpy() 
                    target_q_np = target_q.cpu().numpy()
                    """

                    action_distribution = target_actor.dist(state_batch)        # dim state batch [B , dim obs_space]   
                    actions = action_distribution.sample((N,))                  # dim actions [N , B , dim_action_space]
                    states_expanded = state_batch[None, ...].expand(N, B, dim_state) 
                    # Reshapen von states zu: dim_state = [N*B, dim_obs]
                    # und action zu: dim_action = [N*B, dim_act]
                    target_Q = target_critic.forward(states_epanded.reshape(-1, dim_state),     
                                                    action.reshape(-1, dim_act)).reshape(N,B) # shape N, B
                ...
                def dual(η):
                    # Continuous:
                    max_Q = np.max(target_Q, 0)     # for numerical stability
                    return η * self.ε + np.mean(max_q) \
                        + η * np.mean(np.log(np.mean(np.exp((target_Q - max_Q[None,:]) / η), axis=0))) # without prob since it is already computed for monte carlo sampling
                    ...
                    return η * self.ε + np.mean(max_q) \
                        + η * np.mean(np.log(np.sum(b_log_prob_np * np.exp((target_q_np - max_q) / η), axis=0)))
                    """              
                ...
                # Über dim_0 Mean über N samples berechen um monte carlo approx von pi?
                
                # M-step
                # update policy based on lagrangian
                for _ in range(self.lagrange_it):
                    π_p = self.actor.dist(state_batch)
                    ...
                    # Change to KL for continuous case
