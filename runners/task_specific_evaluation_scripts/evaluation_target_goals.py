...
def evaluate_target_goal(args, actor, eval_env, writer, device, global_step):
    ...
    with torch.no_grad():
        ...
        for _ in range(args.evaluate_episode_num):
            ...
            state, info = eval_env.reset(options={"target_goal": init_goal})
    ...
    metrics = {
        ...
        "eval/episodic_length": mean_len,
    }

    for i in range(num_fixed_positions):
        if returns_by_initial_goal[i]:
            metrics[f"eval/return_init_goal_idx_{i}"] = float(np.mean(returns_by_initial_goal[i]))
