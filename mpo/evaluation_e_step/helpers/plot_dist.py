...
def plot_mpo_weight_distribution(
    ...
    with torch.no_grad():
        ...
        x_first = actions_np[:, b_idx, 0].numpy()
        x_last = actions_np[:, b_idx, -1].numpy()
        y = weights_np[:, b_idx].numpy()
        ...
        for ax, x, dim_name, dim_idx in [
            ...
            # Bereich für kontinuierliche Gaussian-Kurve
            ...
            # Kontinuierliche Gaussian hinter die diskreten Samples
            ax.plot(
                x_dense,
                pdf_dense,
                linestyle="--",
                ...
                alpha=0.9,
                zorder=1,
                label=f"Actor Gaussian (μ={mu_val:.2f}, σ={std_val:.2f})",
            )

            # MPO-Gewichte darüber
            ax.scatter(
                x_plot,
                y_plot,
                alpha=0.8,
    ...
    def _extract_mu_std(mu_dist, std_dist, b_idx, dim_idx):
